...
export const SkillItem = ({icon: Icon, skillName}) => {
  ...
  return (
    <>
      ...
      ${isHovered ? 'bg-[#FFEAD9] text-black scale-105' : 'bg-gray-300 text-gray-800'}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <Icon size={50} className={`mb-4 transition-transform duration-500 ${isHovered ? 'rotate-12 text-black' : 'rotate-0'}`} />
